...
class DB
{
    ...
    public function create_page(int $page, int $size, string $optinal = "")
    {
        $skip = $size * $page;
        $sql = "SELECT * FROM $this->table $optinal LIMIT $size OFFSET $skip";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute();
        $result = $stmt->get_result();
        $data = $result->fetch_all(MYSQLI_ASSOC);
    ...
    public function create_remove(string $where)
    {
        if (!is_null($where)) {
